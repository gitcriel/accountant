...
async function accountExists(validationObject, data, field) {
  if (!validationUtil.isSpecifiedAndCorrectLength(data, field)) return;

  const account = await dbUtil.findOne(queries.FIND_ACCOUNT_BY_EMAIL, [data[field.fieldName]]);
...
function passwordsMatch(validationObject, data, password, confirmPassword, errorMessage) {
  if (!validationUtil.isSpecifiedAndCorrectLength(data, password) ||
     !validationUtil.isSpecifiedAndCorrectLength(data, confirmPassword)) return;

  if (data[password.fieldName] === data[confirmPassword.fieldName]) return;
...
function validateOldPassword(validationObject, account, data, passwordField) {
  if (!validationUtil.isSpecifiedAndCorrectLength(data, passwordField)) return;

  const hashedPassword = hashUtil.hashPassword(data.oldPassword, account.salt);
