...
module.exports = {
  ...
  createSession: async (data) => {
    let username = data[accountMetadata.username.fieldName]
    let password = data[accountMetadata.password.fieldName]

    let account = await dbUtil.findOne(queries.FIND_ACCOUNT_BY_EMAIL, [username])
    await validateSession(account, password)
    
    let token = hashUtil.generateToken()
    let expiryDate = getSessionExpiryDate()
...
function getSessionExpiryDate() {
  let expiryDate = moment().utc().add(15, 'minutes')
...
async function validateSession(account, password) {
  ...
  let hashedpassword = hashUtil.hashPassword(password, account.salt)
