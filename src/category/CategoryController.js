...
module.exports = {
  ...
  getOne: async (account, data, id) => {
    ...
    const category = await dbUtil.getOne(SCHEMA.TABLE_NAME, id);
    if (category == null) {
      const errorToThrow = {...errors.OBJECT_NOT_FOUND, properties: ['Category', id]};
      throw errorToThrow;
    }
...
async function isAuthorized(account, id) {
  const category = await dbUtil.getOne(queries.GET_CATEGORY, [account.id, id]);
  if (category == null) {
