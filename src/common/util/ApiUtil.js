...
module.exports = {
  ...
  apiAuthWrapper: async (req, res, method) => {
    try {
      ...
      let account = await dbUtil.findOne(queries.FIND_ACCOUNT_BY_SESSION_TOKEN, [sessionToken])
      if(account == null) {
        respondWithError(res, errors.HTTP.FORBIDDEN, errors.FORBIDDEN)
        return
      }
...
async function getSessionToken(req) {
  ...
  let session = await dbUtil.findOne(queries.FIND_SESSION_BY_TOKEN, [sessionToken])
  if(session == null)
    return null
