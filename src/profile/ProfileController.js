...
module.exports = {
  create: async (account, data) => {
    ...
    let profile = await dbUtil.create(SCHEMA.TABLE_NAME, 
  ...
  getAll: async (account) => {
    let profiles = await dbUtil.getAllByParent(SCHEMA.TABLE_NAME, SCHEMA.FIELD_ACCOUNT_ID, account.id)
    return profiles
  },
  ...
  getOne: async (account, data, id) => {
    ...
    let profile = await dbUtil.getOne(SCHEMA.TABLE_NAME, id)
    if(profile == null) 
